/**
 * Watermark engine (shared base, pure functions, no UI dependency).
 *
 * ailabel tuning: defaults target the "AI-generated label" scenario —
 * bottom-right badge (br) + 4%-of-long-side font size + 90% opacity +
 * stroke enabled (white text on black stroke, readable on any background).
 * Tile mode remains available for crop-resistance.
 */
...
export const DEFAULT_CONFIG = {
  text: '',            // watermark text, \n splits lines
  mode: 'br',          // tile | center | tl | tr | bl | br (default: bottom-right badge)
  rotate: 0,           // manual rotation (degrees, 90 steps: 0/90/180/270; 90/270 swap canvas w/h)
  density: 3,          // tile density: n x n grid (tile mode only)
  color: '#ffffff',    // text color (hex)
  opacity: 0.9,        // opacity 0..1 (labels must be prominent; default high)
  sizePct: 0.04,       // font size = long side * sizePct (resolution-independent look)
  angle: 45,           // tile rotation angle (degrees)
  fontWeight: 600,
  strokeWidth: 1,      // >0 enables stroke (white on black, readable on light/dark backgrounds)
  strokeColor: '#000000',
};

/**
 * Single source of defaults shared by UI and engine.
 * The engine's DEFAULT_CONFIG (decimal form) is canonical; this derives the
 * percentage form the UI consumes. app.js init/resetAll/readCfg all read
 * from here; index.html value/selected attributes are first-paint fallbacks
 * only (before JS loads).
 */
export const DEFAULTS = Object.freeze({
  ...
  sizePct: Math.round(DEFAULT_CONFIG.sizePct * 1000) / 10, // 0.04 -> 4 (float-tail cleanup)
  purposeIndex: 0,
});
...
export function makeWatermarkedName(originalName, suffix = 'labeled') {
  const d = new Date();
  const ts = `${d.getFullYear()}${pad2(d.getMonth() + 1)}${pad2(d.getDate())}${pad2(d.getHours())}${pad2(d.getMinutes())}`;
  if (!originalName || originalName === 'image.png') return `ai-labeled_${ts}.png`;
